	...
	return declare("bgagame.tichudashwood", ebg.core.gamegui, {
		...
		setup: function(gamedatas) {
			...
			if(gamedatas.mahjongWish > 0) {
				...
				this.prefs[103].value == 1 ? $('mahjongTikiIndicator').style.display='block': $('mahjongIndicator').style.display='block';
			}
			...
			this.phoenixValues = this.setupValueChoice('phoenixChoice', null /*'onPhoenixValuesSelectionChanged'*/, 13);
		...
		setupValueChoice:function (idName, callbackFunc, count) {
				...
				for (var value = 0; value < count; value++) { // Build card type id
		...
		displayLastCombos: function(playerIds, passes) {
				...
				playerIds.forEach((playerId) => {
						...
						if (this.allLastCombos[playerId] != undefined && this.allLastCombos[playerId]['cards'].length > 0) {
		...
		onEnteringStateShowPassedCards: function(args) {
			...
			args._private.forEach((card, i) => {
				...
				dojo.place(
					this.format_block( this.prefs[103].value == 1 ? 'jstpl_tikicardontable': 'jstpl_cardontable', { // x,y = tichu-cards.png (css background-position)
						x: x,
						y: y,
		...
		onEnteringStateChooseDragonGift: function(args) {
			if(this.isCurrentPlayerActive()) {
				...
				this.addActionButton('giveDragonBefore_button', _('Give cards to '+ args.enemies[0]), () => this.onGiveDragon(0));
				this.addActionButton('giveDragonAfter_button', _('Give cards to '+ args.enemies[1]), () => this.onGiveDragon(1));
			}
		},
		...
		onUpdateActionButtons: function(stateName, args) {
			...
			if(!this.isSpectator) {
				if(player.call_grand_tichu < 0) {
					this.addActionButton('noBet', _('No bet'), ()=>this.onGrandTichuBet(0), null, false, 'gray');
					this.addTooltip('noBet', _('Don\'t call Grand Tichu'), '');
					this.addActionButton('makeGTBet', _('Grand Tichu'), ()=>this.onGrandTichuBet(200), null, false, 'red');
		...
		getCardUniqueId: function( color, value ) { // Get card unique identifier based on its color and value
				return (color-1)*14+(value-1);
		},

		getCardValueByTypeID: function(cardTypeID) { //get card value based on it's unique identifier
				return (cardTypeID % 14) + 1;
		},

		getCardColorByTypeID: function(cardTypeID) { //get card color based on it's unique identifier
		...
		setPass: function (playerId) {
				...
				$('lastcombo_' + playerId).innerHTML = "<span class = 'pass'> <img src='" + img + "' width='75px' height='112.5px' alt='pass'> </span>";
				$('cardback_'+playerId).style.display = 'inline-block';
				this.setDescription(playerId, 'Pass');
		},

		setupNewCard: function( card_div, card_type_id, card_id ) {
			if( ( this.getCardValueByTypeID(card_type_id)==10 ) || ( this.getCardValueByTypeID(card_type_id)==13 ) )
		...
		showCurrentTrick: function() {
			...
			myDlg.show();
			let stock = this.createStock($('currentTrickCards'), this.cardwidth * 0.75, this.cardheight * 0.75);
			stock.extraClasses="smallCards";
			stock.setSelectionMode(0);
			this.gamedatas.currentTrick.forEach(card => stock.addToStockWithId(this.getCardUniqueId(card.type, card.type_arg), card.id));
		},
		...
		onGiveCard: function(i) {
			...
			if (this.cardsToPass[i] == null) {
				...
				dojo.place( this.format_block( this.prefs[103].value == 1 ? 'jstpl_tikicardontable': 'jstpl_cardontable', { // x,y = tichu-cards.png (css background-position)
					x: x,
					y: y,
					player_id: player_id,
					card_id: card_id
				...
				this.slideToObjectPos('cardontable_' + player_id + '_' + card_id, 'giveplayertable_' + direction, 0, 0).play();
			} else {
		...
		onPassCards: function () {
			...
			if(this.selectionHandler)
				dojo.disconnect(this.selectionHandler);
			this.cardsToPass = [];
		...
		onReorderByColor: function( evt ) {
			...
			for(var serie = 1; serie <= 4; serie++ ) {
				for(var value = 1; value <= 14; value++ ) {
					var card_type_id = this.getCardUniqueId( serie, value );
					newWeights[card_type_id] = (value == 1)
		...
		*/
		...
		notif_confirmTichu: function( notif ) {
			...
			this.addActionButton('confirmTichu', _('confirm'), () => this.takeAction("confirmTichu", {bet: notif.args.grand ? 200 : 100}));
		},
