...
class PlayerGrid extends APP_GameClass {
	...
	function __construct($bounds, $tiles, $pid) {
		$grid = array_map(fn($r) => array_map(fn($r) => null, range(0,$bounds['right'])), range(0,$bounds['bottom']));
		$positions = [];
	...
	function checkForDiscs(&$result) {
		...
		foreach($positions as $id => $pos) {
			...
			if(count(array_filter($tile['corners'], fn($c) => !is_null($c))) == 0) continue;
			...
			foreach($tile['corners'] as $i => $corner) {
				if(is_null($corner)) continue;
				$needs = [0,0,0,0];
				foreach($corner as $color) $needs[$color]++;
				$success = true;
				for($i < 0; $i < 4; $i++) if($needs[$i] > $couts[$i]) $success=false;
				if($success) {
					$disc = array_shift($supply);
					$disc['location'] = $id;
					$disc['corner'] = $i;
					$result []= $disc;
					if(count($supply) == 0) return true;
				}
			}
		}
		return false;
	...
	function countTiles($color, $tile, &$array) {
		...
		foreach($neighbors as $tile) {
			...
			if(in_array($tile->id, $array)) continue;
			$array []= $tile->id;
	...
	function getFreeSpaces() {
		...
		foreach($grid as $row => $cols) {
			foreach ($cols as $col => $tile) {
				if(!is_null($tile)) continue;
				$nbs = $this->getNeighbors($tile->pos);
				if(count($nbs) > 0) $res []= [$row, $col];
			}
		}
		return $res;
	}
