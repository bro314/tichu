...
class Combo extends APP_GameClass {
	...
	public function __construct($cards, $type=null, $phoenixValue=null, $description=null) {
		if(is_null($type)) {
			...
			foreach (self::$comboNames as $type => $name) {
				...
				if(!is_null($res)) { //combo is of that type
	...
	public function recheckPhoenix() {
		...
		if(in_array($lastCombo->type, [NO_COMBO, DOG_COMBO])) return;
		...
		if($this->phoenixValue[0] > $val) return; // both values can beat it
	...
	public function canBeat($combo) {
		if($combo->type == NO_COMBO || $combo->type == DOG_COMBO) return true; //opening, all can be played
		$length = count($this->cards);
		$cLength = count($combo->cards);

		if($this->type == BOMB_COMBO) {
			if($combo->type == BOMB_COMBO) {
				if($length == $cLength) return $this->getFirstValue() > $combo->getFirstValue();
				return $length > $cLength;
			}
			return true;
		}
		if($this->type != $combo->type || $length != $cLength) return false;
	...
	public function getFirstValue() {
		$card = $this->cards[0];
		$val = $card['type_arg'];
		if ($card['type'] == TYPE_PHOENIX && $val == 1) return $this->phoenixValue;
		if ($card['type'] == TYPE_DRAGON && $val == 1) return 15;
		return $val;
	}
	...
	public function setPhoenixValue($val) {
		...
		if($this->type == STRAIGHT_COMBO) {
			...
			if($val == $this->phoenixValue[1]) { // lower value was chosen
			...
			if($val == $this->phoenixValue[1]) { // just move the first pair to the end
   ...
   * check functions as descriped in the constructor
	...
	public static function checkDog($cards) {
		...
		if($cards[0]['type'] != TYPE_DOG || $cards[0]['type_arg'] != 1) return;
		return [
			'cards' => $cards,
			'phoenixValue' => null,
			'description' => 'Dog'
	...
	public static function checkBomb($cards) {
		if(count($cards) < 4 || $cards[0]['type_arg'] == 1) return;
		$description = null;
		if(count($cards) == 4) {
			if($cards[0]['type_arg'] != $cards[3]['type_arg']) return;
			...
			foreach ($cards as $idx => $card) {
				...
				if($card['type_arg'] != $firstVal + $idx) return;
			}
			$description = "Straight flush bomb starting from "
	...
	public static function checkSingle($cards) {
		...
		$phoenixValue = null;
		$card = $cards[0];
		if($card['type_arg'] == 1 && $card['type'] == TYPE_DOG) return;
		if($card['type_arg'] == 1 && $card['type'] == TYPE_PHOENIX) { //get Value from last combo of this trick
			$lastCombo = LogManager::getLastCombo();
		...
		} else $description = CardManager::cardToStr($card);
	...
	public static function checkPair($cards) {
		...
		if($cards[0]['type_arg'] == 1) {
			if($cards[0]['type'] != TYPE_PHOENIX) return;
			$phoenixValue = $cards[1]['type_arg'];
			if($phoenixValue == 1) return;
		} else {
			if($cards[0]['type_arg'] != $cards[1]['type_arg']) return;
		}
	...
	public static function checkTrip($cards) {
		if(count($cards) != 3) return;
		$phoenixValue = null;
		if($cards[0]['type_arg'] == 1) {
			if($cards[0]['type'] != TYPE_PHOENIX) return;
			$phoenixValue = $cards[1]['type_arg'];
			if($phoenixValue == 1 || $cards[2]['type_arg'] != $phoenixValue) return;
		} else {
			if($cards[0]['type_arg'] != $cards[1]['type_arg']
				|| $cards[0]['type_arg'] != $cards[2]['type_arg'] ) return;
		}
	...
	public static function checkRunningPair($cards) {
		...
		if(!is_null($phoenix)){
			if(is_null($phoenixValue)) { //if the loop founds no gap, it's the last one.
				$phoenixValue = $value;
				$cards[] = $phoenix;
			} else array_splice($cards, $phoenixIndex, 0, [$phoenix]);
		}
	...
	public static function checkStraight($cards) {
		...
		if($cards[0]['type_arg'] == 1) {
			switch($cards[0]['type']) {
			 	case TYPE_PHOENIX:
					if($cards[1]['type_arg'] == 1 && $cards[1]['type'] != TYPE_MAHJONG) return;
					$phoenix = array_shift($cards);
					break;
				case TYPE_MAHJONG:
					break;
		...
		if(!is_null($phoenix)){
			if(is_null($phoenixValue)) { // no gap detected
				$values = [];
				if($value < 15) { // last card was no ace, phoenix can be put at end
					$values[] = $value;
					$phoenixIndex = $count-1;
				}
				if($value > $count+1) { // first card was not 2, phoenix can be put at start
					$values[] = $value-$count;
					$phoenixIndex = 0;
		...
		$highest = CardManager::cardToStr($phoenixIndex == $count-1 ? $phoenixValue : $cards[$count-1]);
	...
	public static function checkFullHouse($cards) {
		if(count($cards) != 5) return;
		$phoenixValue = null;
		$phoenixIndex = null;
		$distinct = array_unique(array_column($cards, 'type_arg')); // get distinct values
		if($cards[0]['type_arg'] == 1) { // special card, check if phoenix
			if($cards[0]['type'] != TYPE_PHOENIX || $cards[1]['type_arg'] == 1) return; //special card other than phoenix
			if(count($distinct) != 3) return;
			$first = $cards[1]['type_arg'];
			$last = $cards[4]['type_arg'];
			if($cards[2]['type_arg'] == $last) {//ABBB
				$phoenixValue=$first;
				$cards = array_reverse($cards);
			} elseif($cards[3]['type_arg'] == $first) { //AAAB
				$phoenixValue = $last;
			...
			if(count($distinct) != 2) return;
			$first = $cards[0]['type_arg'];
			$last = $cards[4]['type_arg'];
			if($cards[1]['type_arg'] != $first) return;
			if($cards[3]['type_arg'] != $last) return;

			if($cards[2]['type_arg'] == $last) $cards = array_reverse($cards); //trip should come first
		}
		...
		return [
			'cards' => $cards,
			'phoenixValue' => $phoenixValue,
			'description' => CardManager::cardToStr($cards[1], true) . " full of " . CardManager::cardToStr($cards[3], true) // idx 1 and 3 can't be phoenix
		];
	}
}
?>
