...
class Hand extends APP_GameClass {
	...
	public function __construct($cards) {
		...
		if($this->cards[0]['type_arg'] == 1 && $this->cards[0]['type'] == TYPE_PHOENIX) $this->phoenix=$this->cards[0];
		elseif(count($this->cards) > 1 && $this->cards[1]['type_arg'] == 1 && $this->cards[1]['type'] == TYPE_PHOENIX) $this->phoenix=$this->cards[1];
	}

	public function test($wish, $combo) {
		if(!in_array($wish, array_column($this->cards, 'type_arg'))) return null;
		//if(is_null($combo)) return null;
		$length = count($combo->cards);
		$name = Combo::$comboNames[$combo->type];
		$highestCombo = /*self::getHighestBomb($this->cards, $wish, $length) ?? */call_user_func("self::getHighest$name" , $this->cards, $wish, $length, $this->phoenix);

		return $highestCombo;
	}

	public function canFulFillWish($wish, $combo) {
		if($combo->type == NO_COMBO || $combo->type == DOG_COMBO) return in_array($wish, array_column($this->cards, 'type_arg'));
		if(!in_array($wish, array_column($this->cards, 'type_arg'))) return false;
		if(is_null($combo)) return true;
		$length = count($combo->cards);
		$name = Combo::$comboNames[$combo->type];
		$highestCombo = self::getHighestBomb($this->cards, $wish, $length) ?? call_user_func("self::getHighest$name" , $this->cards, $wish, $length, $this->phoenix);

		if(is_null($highestCombo)) return false;
		return $highestCombo->canBeat($combo);
	}

	public function hasBomb() {
		if(!is_null($this->containsBomb)) return $this->containsBomb;
		$values = array_column($this->cards, 'type_arg');
		$fours = array_filter($values, function($val, $idx) use ($values){
		...
		for($color = 1; $color <= 4; $color++) {
			$values = array_column(array_filter($this->cards, function($card) use ($color) {return $card['type'] == $color && $card['type_arg'] != 1;})
				, 'type_arg');
	...
	public static function getHighestPair($cards, $wish, $length, $phoenix) {
		...
		$filtered = array_values(array_filter($cards, function($card) use ($wish) {return $card['type_arg'] == $wish;}));
		if($phoenix) {
			return new Combo([$phoenix, $filtered[0]], PAIR_COMBO, $phoenixValue=$wish);
		}
		if(count($filtered) > 1) return new Combo([$filtered[0], $filtered[1]], PAIR_COMBO);
	}
	...
	public static function getHighestTrip($cards, $wish, $length, $phoenix) {
		...
		if($count > 2) return new Combo(array_slice($filtered, 0, 3), TRIP_COMBO);
	}
	...
	public static function getHighestRunningPair($cards, $wish, $length, $phoenix) {
		...
		foreach ($distinct as $idx => $value) {
			if($value==1) continue;
			if($value>$wish) break; // no need to search for running straights that start higher than wish
			if($idx + $off >= $count) break; // not enough values left
			if($value+$off < $wish) continue; // running pair of given length starting here would not contain wish
			// since the distinct values are ordered, if the value at the right position has the right value, so will the values between
			if($distinct[$idx + $off] == $value+$off) $straights[] = $value;
		}
		...
		foreach ($straights as $value) {
			...
			for($i = 0; $i <= $off; $i++) {
				...
				if(!is_null($phoenix) && is_null($phoenixValue)) $phoenixValue = $val;
				else {
					$fail = true;
					break;
				}
			...
			if(!$fail) {
				...
				$cards = array_map(function($val){return ['type_arg' => $val, 'type'=>0];}, $values);
	...
	public static function getHighestStraight($cards, $wish, $length, $phoenix) {
		...
		$count = count($distinct);
		if(is_null($phoenix)) {
			foreach ($distinct as $idx => $value) { //basically the same as for running pairs
				if($value==1) continue;
				if($value>$wish) break;
				if($idx + $off >= $count) break;
				if($value+$off < $wish) continue;
				if($distinct[$idx + $off] == $value+$off) $straight = $value;
			}
		} else {
			foreach ($distinct as $idx => $value) {
				if($value==1) continue;
				if($value>$wish) break;
				if($idx + $off-1 >= $count) break;
				if($value+$off < $wish) continue;
				if($distinct[$idx + $off-1] <= $value+$off) $straight = $value;
			}
			if($straight+$off == 15) $straight--; //happens if the best straight starts with phoenix and ends with ace
		}
		if(is_null($straight)) return;
	...
	public static function getHighestFullHouse($cards, $wish, $length, $phoenix) {
		...
		if(count($doubles) < 2) return; //if no or only 1 value is equal to his successor, there is no full hose
		...
		$fullHouse = null;
		if($phoenix) {
			if($tCount > 0) {// get full houses with phoenix in double
				$tVal = $triples[$tCount-1];
				$dVal = $wish;
				if($dVal == $tVal) {
					$dVal = array_reduce($distinct, function($c,$v) use ($wish) {return $v==$wish ? $c : $v;});
				}
				$fullHouse = [$tVal, $dVal];
			}
			...
			if(!in_array($wish, $distinctDoubles) // the hand contains the wish only once
				|| $tCount==0 // no triple
				|| count($distinctDoubles) < 2) // no double except the triple
					return;
			$tVal = $triples[$tCount-1]; //highest triple
			$dVal = $wish;
			if($dVal == $tVal) { //triple is already the wish, get highest other pair
				$dVal = array_reduce($distinctDoubles, function($c,$v) use ($wish) {return $v==$wish ? $c : $v;});
			}
			if(!is_null($dVal)) $fullHouse = [$tVal,$dVal];
		}
		if(is_null($fullHouse)) return;
		// since we only need the combo to check if it can beat the other, we can create dummy cards
		$cards = [];
		for($i = 0; $i < 3; $i++) $cards[] = ['type_arg' => $fullHouse[0], 'type'=>0];
		for($i = 0; $i < 2; $i++) $cards[] = ['type_arg' => $fullHouse[1], 'type'=>0];
	...
	public static function getHighestBomb($cards, $wish, $l) {
		...
		foreach ($filtered as $card) {
			...
			$values = array_column(array_filter($cards, function($card) use ($color) {return $card['type'] == $color && $card['type_arg'] != 1;})
				, 'type_arg');
			$last = count($values)-1;
			if($last < 4) continue;
			...
			for($idx = 0; $idx < count($values)-4; $idx++) {
				$value = $values[$idx];
				if($value > $wish) break; // no need to search for running straights that start higher than wish
				if($values[$idx+4] != $value+4) continue; // no straight
				// straight of length 5 found! check how long it can be
				$i = $idx+5;
				while($i <= $last && $values[$i] == $value+$i-$idx) $i++;
		...
		// since we only need the combo to check if it can beat the other, we can create dummy cards
		if($length>4)
			return new Combo(array_map(function($v){
				return ['type_arg'=>$v, 'type'=>0];
			}, range($val, $val+$length-1)), BOMB_COMBO);
		if(count($filtered)==4) return new Combo($filtered, BOMB_COMBO);
	}
