...
class CardManager extends APP_GameClass {
	...
	public static function setupCards() {
		$cards = array(); //(0 => 0)
		for( $color = 1; $color < 5; $color++ ) { // swords=1, stars=2, pagodas=3, jade=4
	...
	public static function getPassedCards() {
		$sql = "SELECT card_id id, card_type type, card_type_arg type_arg, card_location_arg location_arg, card_passed_from passed_from FROM card WHERE card_location = 'temporary'";
		$res = self::getObjectListFromDB( $sql );
		$ret = [];
		foreach ($res as $row) {
			$pId = $row['location_arg'];
			if(!isset($ret[$pId])) $ret[$pId] = [];
			$ret[$pId][] = $row;
		}
		return $ret;
	}
	...
	public static function getCardsPassedBy($pId) {
		return self::getObjectListFromDB("SELECT card_id id, card_type type, card_type_arg type_arg FROM card WHERE card_location = 'temporary' AND card_location_arg=$pId");
	}
	...
	public static function cardToStr($card, $plural=false) {
        ...
        case 11:
            if ($plural) return "Jacks";
            return "Jack";
        case 12:
            if ($plural) return "Queens";
            return "Queen";
        case 13:
            if ($plural) return "Kings";
            return "King";
        case 14:
            if ($plural) return "Aces";
            return "Ace";
        default:
            if ($plural) return strval($val) .'&#39;s';
            return strval($val);
			}
	}
	...
	public static function getTrickValue($cards, $asTable=false) {
		$score = 0;
		$table = Utils::map(function($i){return 0;}, range(2,14));
		$special = [];
		foreach( $cards as $card) {
				$val = $card['type_arg'];
				if($val == 1) $special[] = $card['type'];
				else $table[$val]++;
