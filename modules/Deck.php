...
class MyDeck extends APP_GameClass
{
  ...
  function pickCard($from_location, $player_id)
  {
    return self::toCard($this->deck->pickCard($from_location, $player_id));
  }
  ...
  {
    $this->deck->moveCards($cards, $location, $location_arg);
  }
  ...
  function moveAllCardsInLocation(
    ...
    $this->deck->moveAllCardsInLocation(
      $from_location,
      $to_location,
      $from_location_arg,
      $to_location_arg
  ...
  {
    return self::toCard($this->deck->getCard($card_id));
  }
  function getCards($cards_array)
  {
    return self::toCards($this->deck->getCards($cards_array));
  }
  ...
  {
    return self::toCards(array_values($this->deck->getPlayerHand($player_id)));
  }
  function getCardOnTop($location)
  {
    return self::toCard($this->deck->getCardOnTop($location));
  }
  function getCardsOnTop($nbr, $location)
  {
    return self::toCards($this->deck->getCardsOnTop($nbr, $location));
  }
  function getExtremePosition($bGetMax, $location)
  {
    return $this->deck->getExtremePosition($bGetMax, $location);
  }
  ...
  {
    return $this->deck->shuffle($location);
  }
  ...
  function getPlayerGrid($pid)
  {
    ...
    $bounds = self::getObjectFromDB(
      "SELECT max(card_type_arg)+1 bottom, max(card_location_arg)+1 right FROM tile"
    );
  ...
  {
    self::DbQuery(
      "UPDATE tile SET card_location_arg=card_location_arg+1 WHERE card_location='$pId'"
    );
  }
...
class Card extends APP_GameClass
{
  ...
  function __construct($arr)
  {
    ...
    if ($this->location == "wheel" || $this->location == "deck") {
      $this->pos = $arr["location_arg"];
    } else {
      $this->pos = [$arr["type_arg"], $arr["location_arg"]];
    }
  }
  ...
  function store()
  {
    ...
    self::DbQuery(
      "UPDATE tile SET tile_type_arg=$type_arg, tile_location='$location', tile_location_arg=$location_arg WHERE tile_id=$id"
    );
  }
